...
(function () {
  ...
  function prompt(detail) {
    ...
    var moduleOff = detail.error === "module_disabled";
    ...
    box.innerHTML =
      ...
      '<div style="font-size:18px;font-weight:700;margin-bottom:6px">' + (moduleOff ? "Module switched off" : manager ? "No access" : "Sign in required") + "</div>" +
      '<p style="margin:0 0 16px;color:var(--mut,#555)">' +
      (moduleOff ? String(detail.message || "This module isn't switched on.").replace(/[<>&]/g, "")
      ...
      (moduleOff ? '<a href="compliance" style="display:block;text-align:center;padding:12px;border-radius:10px;background:var(--acc,#0369a1);color:var(--on-acc,#fff);text-decoration:none;font-weight:600">Back to Compliance</a>' : manager ? "" : '<a href="/" style="display:block;text-align:center;padding:12px;border-radius:10px;background:var(--acc,#0369a1);color:var(--on-acc,#fff);text-decoration:none;font-weight:600">Sign in to DH FleetView</a>') +
    ...
    return nativeFetch(input, init).then(function (res) {
      if (sameOrigin && (res.status === 401 || res.status === 403)) {
        res.clone().json().then(function (body) {
          ...
          if (d && (d.error === "login_required" || d.error === "not_manager" || d.error === "module_disabled")) {
